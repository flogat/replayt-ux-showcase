...
**Source of truth:** [`docs/examples/PATTERNS.md`](examples/PATTERNS.md) — pattern IDs (**P-01**–**P-05** and **P-09** vanilla **Shipped**; **P-10** **Spec only**; **P-06**–**P-08** framework rows, …), filenames, status (**Shipped** / **Spec only** / **Planned**), and per-pattern acceptance criteria.

| Metric | Target | Current (update when patterns ship) |
| ------ | ------ | ------------------------------------- |
| Distinct vanilla patterns in `docs/examples/` | ≥ 5 | **6** shipped (**P-01**–**P-05**, **P-09** — [`PATTERNS.md`](examples/PATTERNS.md)), including **`fixture-replay.html`** (**P-05**: deterministic offline fixture for reviewers / **LLM** harnesses) and **`event-overlay.html`** [**P-09**](examples/PATTERNS.md#p-09--event-overlay-lane-scrub-hover-tooltips-keyboard) (event overlay lane + scrub-linked callouts; normative acceptance in **PATTERNS.md**). **P-10** — [**Spec only**](examples/PATTERNS.md#p-10--click-heatmap-on-static-canvas-session-click-coordinates): planned **`click-heatmap-canvas.html`** (session **click** **`x`/`y`** heatmap on a viewport-sized **canvas** / **SVG** stage; distinct from **P-09**). |
