...
| **Vanilla examples** (`docs/examples/`) | Intended copy-paste surface per [Showcase stack matrix](DESIGN_PRINCIPLES.md#showcase-stack-matrix); **replayt** pins must sit inside the same PEP 508 range as `pyproject.toml` | Contract test **`tests/test_docs_examples_replayt_pins.py`** (per [Vanilla examples: integrator-facing replayt pins](DESIGN_PRINCIPLES.md#vanilla-examples-integrator-facing-replayt-pins)); headless **[Playwright](DESIGN_PRINCIPLES.md#docs-examples-static-demos-playwright-smoke-tests)** smoke **`tests/docs_examples_playwright/test_basic_player.py`** on **`basic-player.html`** (**P-01**) in job **`docs-examples-playwright`** (`.github/workflows/ci.yml`, **Python** **3.12**) | Keeps CDN and requirement snippets from drifting ahead of the supported consumer story; intentional out-of-range demos use `<!-- replayt-examples:pin-exempt -->` per design principles. **CDN** delivery, optional **SRI**, and **bundler** alternatives: [`docs/FRONTEND_SUPPLY_CHAIN.md`](FRONTEND_SUPPLY_CHAIN.md). Optional maintainer **npm** + **Vite** / **esbuild** preview recipe: [`docs/examples/build.md`](examples/build.md). |
...
| ------- | ---------------------------------------- | ------------------------------ |
| **Install + tests** | Editable install with **`pip install -e ".[dev]"`**, then **`python -m pytest tests --ignore=tests/docs_examples_playwright`** so `[tool.pytest.ini_options]` (including **pytest-cov** options) applies to the non-browser suite | Same command reproduces the **demo.py** coverage gate locally; **Playwright** smoke is a second step (see **README**). |
...
| **Playwright** smoke (**P-01**) | Job **`docs-examples-playwright`**: headless **Chromium**, **`tests/docs_examples_playwright/`**, **`--no-cov`** per [Playwright smoke spec](DESIGN_PRINCIPLES.md#docs-examples-static-demos-playwright-smoke-tests) | **Linux** **CI** only for this row today (**Python** **3.12**); local runs need **`playwright install chromium`** (or **`--with-deps`** on **Linux**). |
