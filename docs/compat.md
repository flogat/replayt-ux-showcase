...
| **Vanilla examples** (`docs/examples/`) | Intended copy-paste surface per [Showcase stack matrix](DESIGN_PRINCIPLES.md#showcase-stack-matrix); **replayt** pins must sit inside the same PEP 508 range as `pyproject.toml` | Contract test **`tests/test_docs_examples_replayt_pins.py`** (per [Vanilla examples: integrator-facing replayt pins](DESIGN_PRINCIPLES.md#vanilla-examples-integrator-facing-replayt-pins)); **[Playwright](DESIGN_PRINCIPLES.md#docs-examples-static-demos-playwright-smoke-tests)** smoke on **`basic-player.html`** (**P-01**) when shipped (see stack matrix *Headless browser smoke* row) | Keeps CDN and requirement snippets from drifting ahead of the supported consumer story; intentional out-of-range demos use `<!-- replayt-examples:pin-exempt -->` per design principles. **CDN** delivery, optional **SRI**, and **bundler** alternatives: [`docs/FRONTEND_SUPPLY_CHAIN.md`](FRONTEND_SUPPLY_CHAIN.md). Optional maintainer **npm** + **Vite** / **esbuild** preview recipe: [`docs/examples/build.md`](examples/build.md). |
...
| **Playwright** smoke (**P-01**) | When shipped: headless **Chromium** job per [Playwright smoke spec](DESIGN_PRINCIPLES.md#docs-examples-static-demos-playwright-smoke-tests) | Same rule as other rows—do not claim **PR** **CI** coverage until `.github/workflows/` runs it (or document **dispatch-only** in **README** / here). |
