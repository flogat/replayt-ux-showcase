...
| **Vanilla HTML load smoke** | **Not** in **`ci.yml` today** — spec: [Static HTML examples: browser smoke (Playwright)](DESIGN_PRINCIPLES.md#static-html-examples-browser-smoke-playwright) | When a job lands, add a stable **EX-*** row to [CI exercise row inventory](#ci-exercise-row-inventory) in the **same** change set as **`ci.yml`**. |
