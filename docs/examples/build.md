...
- **Integrator default (shipped HTML):** **P-11** uses the **Tailwind Play CDN** in **`basic-player-tailwind.html`** for copy-paste demos only (see the visible build note in that file). The in-file note should carry concrete scan examples for both repo-root copies (for example **`"./docs/examples/basic-player-tailwind.html"`**) and relocated app copies (for example **`"./src/**/*.{html,js,ts,jsx,tsx}"`**) so class lists survive purge in a real build.
...
- **Tailwind syntax examples:** If you expand this note later, keep one example each for **v3** `content`, **v4** `@source`, and CLI **`--content`** so integrators can translate the shipped HTML comment into their stack without inferring missing syntax.
