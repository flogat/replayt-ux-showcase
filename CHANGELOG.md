...
- Phase **2** spec (*Offline deterministic fixture page for LLM and reviewer workflows*): **`docs/examples/PATTERNS.md`** — **P-05** (**Spec only**) normative contract for **`docs/examples/fixture-replay.html`** (inlined synthetic **`sessionData`**, no session over the wire, no secrets, no live/stochastic model calls, pinned **replayt** script + local static-server open instructions); **`docs/DESIGN_PRINCIPLES.md`** — acceptance traceability, **[LLM boundaries](docs/DESIGN_PRINCIPLES.md#llm-boundaries)** row for **P-05**, **Audience** (**automation agents**); **`README.md`** (reviewer / harness section + layout table); **`docs/REPLAYT_ECOSYSTEM_IDEA.md`**; **`docs/MISSION.md`** / **`docs/compat.md`** pattern summaries. **HTML** implementation deferred to **Builder** phase **3**.
