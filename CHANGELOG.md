...
- **Playwright** smoke for **`docs/examples/basic-player.html`** (**P-01**): **`playwright`** and **`pytest-playwright`** in **`[project.optional-dependencies].dev`**, **`tests/docs_examples_playwright/`** (session **HTTP** server rooted at **`docs/examples/`**, **`test_basic_player.py`**), **CI** job **`docs-examples-playwright`** (**`playwright install --with-deps chromium`**, **`pytest … --no-cov --browser chromium`**); main **`test`** job uses **`pytest tests --ignore=tests/docs_examples_playwright`** so the **`demo.py`** **pytest-cov** gate is unchanged (backlog phase **3**, *Add Playwright smoke tests for docs/examples static demos*).
...
- Phase **2** spec (*Add Playwright smoke tests for docs/examples static demos*): **`docs/DESIGN_PRINCIPLES.md`** — [docs/examples static demos: Playwright smoke tests](docs/DESIGN_PRINCIPLES.md#docs-examples-static-demos-playwright-smoke-tests) (serving **`docs/examples/`** over **HTTP**, **P-01** / **`basic-player.html`** invariants, **Chromium**-first **CI**, isolation from **`demo.py`** **pytest-cov** gate); **Showcase stack matrix** *Headless browser smoke* row; **GitHub Actions CI workflow** table row; traceability and **compat.md** / **README** cross-links.
